...
cobs_ret_t cobs_encode(void const* dec, unsigned dec_len, void* out_enc,
											 unsigned enc_max, unsigned* out_enc_len) {
	if (!out_enc_len) {
		return COBS_RET_ERR_BAD_ARG;
	}
...
cobs_ret_t cobs_encode_inc_begin(void* out_enc, unsigned enc_max,
...
cobs_ret_t cobs_encode_inc(cobs_enc_ctx_t* ctx, void const* dec,
													 unsigned dec_len) {
	if (!ctx || !dec) {
		return COBS_RET_ERR_BAD_ARG;
	}
...
cobs_ret_t cobs_decode(void const* enc, unsigned enc_len, void* out_dec,
											 unsigned dec_max, unsigned* out_dec_len) {
	...
	if ((src[0] == COBS_FRAME_DELIMITER) ||
			(src[enc_len - 1] != COBS_FRAME_DELIMITER)) {
		return COBS_RET_ERR_BAD_PAYLOAD;
	}
